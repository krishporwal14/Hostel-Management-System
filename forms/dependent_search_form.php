    ...
    <div class="min-h-screen bg-neutral-950 text-white pt-5">
        <div class="bg-zinc-800 mx-auto p-4 rounded-lg container">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold">Search Dependent's Details</h2>
            ...
            <form action="#" id="searchForm" method="POST" class="flex flex-col max-w rounded px-8 pt-6 pb-2">
                ...
                <div>
                    <button type="submit" onclick="submitSearchForm()" class="bg-cyan-800 hover:bg-cyan-900 text-white w-full font-bold py-3 px-4 rounded-lg focus:outline-none focus:shadow-outline">Search Dependent</button>
                </div>
            </form>
            <div class="m-4">
                <?php
                ...
                try {
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        $sql = "SELECT * FROM dependents WHERE student_id='$student_id';";
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo '<p class="text-md"><strong>Name:</strong> ' . $row['name'] . ' Rs.' . '</p>';
                                echo '<p class="text-md"><strong>Age:</strong> ' . $row['age'] . '</p>';
                                echo '<p class="text-md"><strong>Relation:</strong> ' . $row['relation'] . ' Rs.' . '</p>';
                                echo '</div>';
                            }
                        } else {
